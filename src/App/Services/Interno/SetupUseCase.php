 ...
 *
 * @see ISetupUseCase The contract this implements, and why the door is safe.
 * @uses IUnitOfWork The single boundary the whole bootstrap runs in.
 * @uses IUserRepository Asked whether any user exists, then written to.
 * @uses IRoleRepository Receives the role granting everything registered.
 *
 * @license {@link https://opensource.org/licenses/MIT MIT}
 * @copyright 2026 Tachyon
 *
...
{
    /**
     * @var string Name of the role the first user is given. Fixed rather than
     *             asked for — the operator is setting up a system, not naming
     *             one.
     */
    private const string ROLE_NAME = 'Administrator';

    /**
     * Takes no registrar: there is no permission to declare, and could not be.
     *
     * Six collaborators, more than any other use case here, because bootstrapping
     * touches every part of the identity model at once.
     *
     * @param  IUnitOfWork  $unitOfWork  The single boundary the whole bootstrap
     *                                   runs in.
     * @param  IUserRepository  $users  Asked whether any user exists — the guard
     *                                  — then written to.
     * @param  IRoleRepository  $roles  Receives the administrator role.
     * @param  IPermissionRepository  $permissions  Read in full, so the role
     *                                              grants whatever the running
     *                                              build registered rather than
     *                                              a hard-coded list.
     * @param  IUserTM  $userTM  Validates and builds the user, hashing the
     *                           password.
     * @param  IRoleTM  $roleTM  Validates and builds the role.
     *
     * @copyright 2026 Tachyon
     */
