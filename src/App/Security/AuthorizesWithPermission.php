...
/**
 * Authorizes With Permission Trait.
 *
 ...
 *
 * Not every use case uses this trait: the unguarded ones — login, session
 * validation, setup — have no permission to declare, because there is no caller
 * yet to check one against.
 *
 * @see UserContext Who is being checked.
 * @see IRegisterPermissionUseCase Where the slug is declared.
 * @see \App\Services\Interno\CreateProductUseCase A representative user of it.
 *
 * @license {@link https://opensource.org/licenses/MIT MIT}
 * @copyright 2026 Tachyon
 */
...
{
    /**
     * @var string The slug returned by the registry; the sole thing the use case
     ...
     *
     * @param  IRegisterPermissionUseCase  $registrar  The registration use case,
     *                                                 taken as an argument
     *                                                 rather than stored — it is
     *                                                 wanted once, at boot.
     * @param  string  $slug  `domain:action`, and the use case's own to choose.
     ...
     * ```
     *
     * The message is deliberately the same whoever is refused and whatever they
     * were refused: telling a caller *which* permission they lack describes the
     * shape of the system to someone who has just been told they may not see
     * it.
     *
     * @param  UserContext  $context  The caller, as the command carried them.
