...
/**
 * Permission List Response Factory.
 *
 * @category API
 *
...
use API\Fbs\Metadata\PermissionListResponse;
use API\Negociation\IResponseAbstractFactory;
use Google\FlatBuffers\FlatbufferBuilder;
use Shared\Exceptions\Result;

/**
 * Renders a {@see PermissionListXResponse} in either wire format.
 *
 ...
 * @copyright 2026 Tachyon
 */
final readonly class PermissionListXResponseFactory implements IResponseAbstractFactory
{
    /**
    ...
    /**
     * @param  PermissionListXResponse  $message  The message to render.
     *
     * @copyright 2026 Tachyon
     */
    public function __construct(private PermissionListXResponse $message)
    {
        $this->dataFactories = array_map(
            static fn (MetadataItemXResponse $item): IResponseAbstractFactory => new MetadataItemXResponseFactory($item),
            $message->data,
        );
    }
    ...
    public function createFlatbuffer(FlatbufferBuilder $builder): Result
    {
        ...
        $data = PermissionListResponse::createDataVector($builder, $itemOffsets);

        return Result::success(PermissionListResponse::createPermissionListResponse($builder, $data));
    }
