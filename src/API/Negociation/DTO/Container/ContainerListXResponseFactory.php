...
/**
 * Container List Response Factory.
 *
 * @category API
 *
...
use API\Fbs\Container\ContainerListResponse;
use API\Negociation\IResponseAbstractFactory;
use Google\FlatBuffers\FlatbufferBuilder;
use Shared\Exceptions\Result;

/**
 * Renders a {@see ContainerListXResponse} in either wire format.
 *
 ...
 * @copyright 2026 Tachyon
 */
final readonly class ContainerListXResponseFactory implements IResponseAbstractFactory
{
    /**
    ...
    /**
     * @param  ContainerListXResponse  $message  The message to render.
     *
     * @copyright 2026 Tachyon
     */
    public function __construct(private ContainerListXResponse $message)
    {
        $this->dataFactories = array_map(
            static fn (ContainerXResponse $item): IResponseAbstractFactory => new ContainerXResponseFactory($item),
            $message->data,
        );
    }
    ...
    public function createFlatbuffer(FlatbufferBuilder $builder): Result
    {
        ...
        return Result::success(ContainerListResponse::createContainerListResponse(
            $builder,
            $data,
            $nextCursor,
            $this->message->total,
