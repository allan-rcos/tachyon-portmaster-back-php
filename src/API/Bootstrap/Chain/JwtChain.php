...
/**
 * Reads the session token variables into a {@see JwtConfig}.
 *
 * The one link that can fail the boot outright: every other variable falls back
 * to a default, but a too-short signing secret is refused here rather than
 * carried forward.
 *
 * @see ApiChain The link shape this otherwise follows.
 *
 * @license {@link https://opensource.org/licenses/MIT MIT}
 * @copyright 2026 Tachyon
 */
...
{
    /**
     * Shortest signing secret this will accept, in bytes.
     *
    ...
    /**
     * Fills the draft's `jwt` slot from the `APP_JWT_*` and `APP_REFRESH_*`
     * variables.
     *
     * Unlike the other links, this one throws instead of recording an error:
     * a weak secret is not a value to report alongside the others, it is a
     * reason not to serve requests at all.
     *
     * @param  EnvSource  $env  Reader over the loaded environment.
     * @param  BootDraft  $draft  Accumulator to write the JWT group into.
     *
     * @throws RuntimeException When `APP_JWT_SECRET` is shorter than
