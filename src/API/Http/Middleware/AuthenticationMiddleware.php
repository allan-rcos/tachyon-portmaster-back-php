 ...
 *
 * @see \API\Http\Session What reads what this establishes.
 * @uses ITokenService Verifies the token and rebuilds the context.
 * @uses AuthCookie Reads and clears the cookie.
 * @uses ILogger Binds the caller's id to the request's lines.
 *
 * @license {@link https://opensource.org/licenses/MIT MIT}
 * @copyright 2026 Tachyon
 */
final class AuthenticationMiddleware implements MiddlewareInterface
{
    ...
    /**
     * @param  ITokenService  $tokenService  Verifies the access token.
     * @param  AuthCookie  $cookie  Reads it off the request, and clears it.
     * @param  ILogger  $logger  Narrowed to the `auth` channel.
     *
     * @copyright 2026 Tachyon
     */
    ...
    /**
     * Resolves the caller, if there is one, and runs the stack either way.
     *
     * Three paths, none of which rejects the request: no cookie proceeds
     * anonymously; a cookie that fails verification proceeds anonymously *and*
     * is cleared on the response; a valid one publishes the context. Enforcing
     * is somebody else's job — see the class note.
     *
     * @param  ServerRequestInterface  $request  The incoming HTTP request.
     * @param  RequestHandlerInterface  $handler  The rest of the stack.
     * @return ResponseInterface The inner response, plus a clearing `Set-Cookie`
     *                           when the presented token was unusable.
     *
     * @copyright 2026 Tachyon
     *
     * @api
