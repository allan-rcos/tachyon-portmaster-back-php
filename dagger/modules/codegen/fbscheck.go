...
// CheckFbsGo falha se os bindings Go commitados estiverem defasados em relação
// aos schemas.
//
// Compara com `diff -r` em vez de `git diff --exit-code`, e a diferença
// importa: assim a checagem não precisa do .git dentro do container — que é
// justamente o que o `+ignore` mantém de fora — e não suja a árvore de trabalho
// de quem rodar (R6.1).
