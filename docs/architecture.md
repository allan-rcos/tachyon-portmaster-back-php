...
2. **Middleware stack** (`ApiProvider::router()`), outermost first:
   ...
   - `LoggingMiddleware`.
   - `FlatBufferNegotiationMiddleware` — decides binary vs JSON from `Accept`
     and `Content-Type`.
...
6. **Response** — the controller fills a `*ResponseProxy` and returns it;
   `ResponseEmitter` writes binary or JSON depending on what the negotiation
   middleware decided.
...
schemas in the `swagger/` submodule. The generated classes are never edited —
each has a hand-written `*Proxy` beside it that adds JSON coercion and the
build/parse helpers the application uses. See
[ADR 0001](adr/0001-flatbuffers-over-json.md) and
