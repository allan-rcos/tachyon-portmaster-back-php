 ...
 *
 * Takes no constructor arguments: there is nothing to filter, and every caller
 * asks the same question.
 *
 * One statement rather than eight, so all the numbers describe the same instant.
 * Even the fixed status literals are bound rather than interpolated — the layer
 * builds no SQL by concatenation, whether or not the value came from a caller.
 *
 * @see MetricsView What it hydrates to.
 * @see \Domain\Enums\ContainerStatus The statuses the occupancy breakdown counts.
 *
 * @license {@link https://opensource.org/licenses/MIT MIT}
 * @copyright 2026 Tachyon
 *
...
final readonly class MetricsDQL implements IDQL
{
     ...
     * hands back the placeholder to splice into the sub-select, so every binding
     * lives on the single statement the runner executes.
     *
     * @return SqlQuery The statement and its bindings — one per status
     *                  comparison.
     *
     * @copyright 2026 Tachyon
     */
    ...
    /**
     * Reads the single row into the view.
     *
     * A result set with no row hydrates to a view of zeroes rather than to null:
     * the aggregates always produce a row against a real database, so an absent
     * one means something is wrong upstream, and a dashboard of zeroes is a
     * safer answer than a type error.
     *
     * @param  list<array<string, mixed>>  $rows  Exactly one row, in practice.
     * @return MetricsView The snapshot.
     *
     * @copyright 2026 Tachyon
     */
    ...
    /**
     * Coerces one aggregate column to an int.
     *
     * @param  mixed  $value  The column as the driver returned it; MySQL hands
     *                        `COUNT(*)` back as a string.
     * @return int The value, or zero when it was not numeric.
     *
     * @copyright 2026 Tachyon
     */
