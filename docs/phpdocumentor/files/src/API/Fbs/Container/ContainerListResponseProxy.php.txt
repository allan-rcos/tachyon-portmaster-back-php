...
/**
 * JSON/binary-aware proxy around the generated {@see ContainerListResponse} table.
 *
 ...
 * @copyright 2026 Tachyon
 */
final class ContainerListResponseProxy extends ContainerListResponse implements IFbsProxy
{
    use CoercesJson;

    /**
     * @param  list<ContainerResponseProxy>  $data
     *
     * @copyright 2026 Tachyon
     */
    ...
    {
        $itemOffsets = array_map(fn(ContainerResponseProxy $item) => $item->buildInto($builder), $this->data);
        $data        = ContainerListResponse::createDataVector($builder, $itemOffsets);
     ...
     *
     * @param  ContainerListResponse  $table  The generated table to read.
     ...
     * @api
     */
    public static function fromTable(ContainerListResponse $table): static
    {
        $data = [];
        for ($i = 0, $n = $table->getDataLength(); $i < $n; $i++) {
            $data[] = ContainerResponseProxy::fromTable($table->getData($i));
        }
    ...
    {
        return static::fromTable(ContainerListResponse::getRootAsContainerListResponse(ByteBuffer::wrap($binary)));
    }
    ...
    {
        return new static(
            data: array_map(static fn (array $row): ContainerResponseProxy => ContainerResponseProxy::jsonUnserialize($row), self::jsonRows($data, 'data')),
    ...
    {
        return [
            'data'        => array_map(fn(ContainerResponseProxy $item) => $item->jsonSerialize(), $this->data),
