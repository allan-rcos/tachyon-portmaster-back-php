...
-- already tracks what ran in schema_migrations, so this is not what protects the
-- normal path — it protects the abnormal one, where someone pipes a migration
-- straight into mariadb to inspect or repair a database mid-incident. Failing
-- there with "table already exists" tells the operator nothing and leaves the
-- rest of the file unapplied. The matching .down.sql is symmetric, using
-- IF EXISTS for the same reason. See db/README.md.
